...
(function (global) {
  ...
  function resolveStudyUser() {
    try {
      const viaApi = global.YogabrataAuth?.getUser?.();
      if (viaApi?.name && !isDemoProfile(viaApi)) {
        return {
          id: viaApi.id || viaApi.email || DEFAULT_USER.id,
          name: viaApi.name,
          grade: viaApi.grade || '10',
        };
      }
      const saasRaw = global.localStorage?.getItem('ml_student_user');
      if (saasRaw) {
        const saasUser = JSON.parse(saasRaw);
        if (saasUser?.name && !isDemoProfile(saasUser)) {
          return {
            id: saasUser.id || saasUser.email || DEFAULT_USER.id,
            name: saasUser.name,
            grade: saasUser.grade || '10',
          };
        }
      }
    } catch {
