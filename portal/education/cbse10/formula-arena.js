/*
  ...
  Formula Battle Arena — turn-based concept RPG on the CBSE Class 10 syllabus.
...
(function () {
  ...
  /* Boss roster — each guards a slice of the syllabus. */
  var BOSSES = [
    {
      id: 'voltaron', name: 'Voltaron', title: 'Circuit Warden', emoji: '⚡',
      tint: '#fbbf24', glow: 'rgba(251,191,36,0.5)',
      hp: 100, power: 12, subject: 'mixed', maxDifficulty: 1,
      taunt: 'Current flows through my veins. Can you follow the circuit?',
    },
    {
      id: 'trigonos', name: 'Trigonos', title: 'Angle Sovereign', emoji: '📐',
      tint: '#f472b6', glow: 'rgba(244,114,182,0.5)',
      hp: 130, power: 15, subject: 'mathematics', maxDifficulty: 2,
      taunt: 'Every angle is mine. Prove your ratios.',
    },
    {
      id: 'acidus', name: 'Acidus', title: 'Reaction Alchemist', emoji: '🧪',
      tint: '#34d399', glow: 'rgba(52,211,153,0.5)',
      hp: 155, power: 18, subject: 'science', maxDifficulty: 2,
      taunt: 'One wrong equation and you dissolve.',
    },
    {
      id: 'quadrix', name: 'Quadrix', title: 'Root Breaker', emoji: '🔻',
      tint: '#a78bfa', glow: 'rgba(167,139,250,0.5)',
      hp: 180, power: 21, subject: 'mathematics', minDifficulty: 2,
      taunt: 'My discriminant is negative. You have no real hope.',
    },
    {
      id: 'chaos', name: 'Chaos Prime', title: 'Syllabus Overlord', emoji: '👁️',
      tint: '#fb7185', glow: 'rgba(251,113,133,0.55)',
      hp: 220, power: 26, subject: 'mixed', minDifficulty: 2,
      taunt: 'Science and Mathematics both bow to me. Show me everything.',
    },
  ];
  ...
  function cacheDom() {
    ['heroHp', 'heroFill', 'heroText', 'foeHp', 'foeFill', 'foeText', 'foeAvatar',
     'foeName', 'foeTitle', 'heroAvatar', 'combo', 'stage', 'timerFill', 'tags',
     'question', 'options', 'feedback', 'powers', 'overlay', 'overlayBody',
     'nextBtn', 'fleeBtn', 'roundNo', 'score', 'bossNo', 'flash'].forEach(function (k) {
      el[k] = $('fa-' + k);
    });
  }
  ...
  function heroDamage(q, secondsLeft, combo) {
    var base = 12 + q.difficulty * 6;
    var speed = Math.round((secondsLeft / TURN_SECONDS) * 10);
    var comboBonus = Math.min(combo, 5) * 3;
  ...
  /* ── HP rendering ──────────────────────────────────────────────── */
  function renderHp() {
    var hp = Math.max(0, state.heroHp);
    var hpPct = (hp / HERO_HP) * 100;
    ...
    el.heroText.textContent = hp + ' / ' + HERO_HP + ' HP';

    var boss = state.boss;
    var bhp = Math.max(0, state.foeHp);
    var bPct = (bhp / boss.hp) * 100;
    el.foeFill.style.width = bPct + '%';
    el.foeFill.classList.toggle('is-mid', bPct <= 55 && bPct > 28);
    el.foeFill.classList.toggle('is-low', bPct <= 28);
    el.foeText.textContent = bhp + ' / ' + boss.hp + ' HP';
  ...
  function nextRound() {
    ...
    state.round += 1;
    el.roundNo.textContent = state.round;   // keep the chip in step from round 1
    el.feedback.classList.remove('is-on');
    el.nextBtn.hidden = true;

    if (!state.queue.length) {
      state.queue = Bank.draw({ count: 20, subject: state.boss.subject });
    }
    ...
    el.tags.innerHTML = '';
    [[q.subjectLabel, q.subject], [q.chapterTitle, 'chapter'],
     [['', 'Easy', 'Medium', 'Hard'][q.difficulty], 'diff']].forEach(function (t) {
    ...
    q.options.forEach(function (text, i) {
      ...
      b.type = 'button';
      b.className = 'fa-opt';
      b.innerHTML = '<span class="fa-opt-key">' + 'ABCD'[i] + '</span><span>' + escapeHtml(text) + '</span>';
  ...
  function heroStrikes(q) {
    ...
    showFeedback(true,
      (isCrit ? '💥 CRITICAL HIT! ' : '⚔️ Direct hit! ') + dmg + ' damage. ' + q.explain);
  }
  ...
  function showFeedback(ok, text) {
    ...
    el.feedback.innerHTML = '<b>' + (ok ? 'Correct' : 'Incorrect') + '</b>' + escapeHtml(text);
    el.feedback.classList.add('is-on');
  }

  /* ── Outcomes ──────────────────────────────────────────────────── */
  function grantRewards() {
    // Two random power-ups per victory, plus a guaranteed shield from boss 3 on.
    var picks = Bank.shuffle(POWERS).slice(0, 2).map(function (p) { return p.id; });
  ...
  function bossDefeated() {
    ...
    var last = state.bossIndex >= BOSSES.length - 1;
    if (last) return victory();

    var rewards = grantRewards();
    openOverlay(
      '<span class="fa-card-emoji">🏅</span>' +
      '<h2>' + state.boss.name + ' defeated!</h2>' +
      '<p>You cleared the ' + state.boss.title + ' with <b style="color:#34d399">' +
      ...
      '<p style="font-size:0.83rem">Next up: <b style="color:' + BOSSES[state.bossIndex + 1].tint + '">' +
        BOSSES[state.bossIndex + 1].emoji + ' ' + BOSSES[state.bossIndex + 1].name + '</b> — ' +
        escapeHtml(BOSSES[state.bossIndex + 1].taunt) + '</p>' +
  ...
  function victory() {
    ...
    openOverlay(
      ...
      '<p>You defeated all ' + BOSSES.length + ' concept bosses and mastered the Class 10 syllabus.</p>' +
      '<div class="fa-scoreline">' +
  ...
  function defeat() {
    ...
    openOverlay(
      '<span class="fa-card-emoji">💔</span>' +
      '<h2>Defeated by ' + state.boss.name + '</h2>' +
      '<p>' + escapeHtml(state.boss.taunt) + '</p>' +
      '<div class="fa-scoreline">' +
      ...
      '</div>' + recapHtml() +
      '<div class="fa-actions">' +
      '<button type="button" class="fa-btn" data-act="retry">Retry ' + state.boss.name + '</button>' +
    ...
    return '<div class="fa-recap"><h3>Revise these ' + state.missed.length + ' concepts</h3><ul>' +
      state.missed.slice(-8).map(function (q) {
        return '<li><b>' + escapeHtml(q.question) + '</b><em>' +
          escapeHtml(q.options[q.answer]) + '</em> — ' + escapeHtml(q.chapterTitle) + '</li>';
      }).join('') + '</ul></div>';
  }
  ...
  function saveBest() {
    try {
      var prev = parseInt(localStorage.getItem('cbse10_arena_best') || '0', 10);
      if (state.score > prev) localStorage.setItem('cbse10_arena_best', String(state.score));
    } catch (e) { /* private mode — scores just do not persist */ }
  }

  function bestScore() {
    try { return parseInt(localStorage.getItem('cbse10_arena_best') || '0', 10); }
    catch (e) { return 0; }
  }
  ...
  function showStartCard() {
    ...
    openOverlay(
      ...
      '<h2>Formula Battle Arena</h2>' +
      '<p>Five concept bosses guard the Class 10 syllabus. Answer correctly to strike; ' +
      'answer wrong and they strike you. Speed and combos multiply your damage.</p>' +
      '<div class="fa-roster">' + BOSSES.map(function (b, i) {
        return '<div class="fa-roster-item"><i>' + b.emoji + '</i><b>' + b.name + '</b>' +
               '<span>' + b.hp + ' HP</span></div>';
      }).join('') + '</div>' +
